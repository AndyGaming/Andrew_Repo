...
class Tree:
    ...
    '''
        ...
        if itr < 1 or itr > 5:
            pm.confirmDialog(t='Error', m='Number of iterations must be between 1 and 5', 
                            icn='critical', b='OK', db='OK', cb='OK')
            self.can_draw = False
                    
        if '=' in axiom:
            pm.confirmDialog(t='Error', m='Invalid input for axiom', 
                            icn='critical', b='OK', db='OK', cb='OK')
            self.can_draw = False
        
        for i in range(0, len(rules)):
            if not rules[i][1] == '=':
                pm.confirmDialog(t='Error', m='Invalid input for rule {0}'.format(i+1), 
                                icn='critical', b='OK', db='OK', cb='OK')
                self.can_draw = False
	...
	'''
        self.get_variables()
        string = self.set_str(self.axiom, self.set_dict(), self.iterations)
        pos_stack = []
        rot_stack = []
        ...
        if self.can_draw:
            ...
            for i in string:
                if i == '+':
                    if self.is_flat:
                        pm.rotate(-self.angle, 0, 0, r=True, os=True, fo=True)
                    else:
                        pm.rotate(-self.rand(self.angle), self.angle, 0, r=True, os=True, fo=True)
                elif i == '-':
                    if self.is_flat:
                        pm.rotate(self.angle, 0, 0, r=True, os=True, fo=True)
                    else:
    ...
    def make_ui(self):
        ...
        self.window = pm.window(window_name, 
                           title=window_title,
