...
class DotBot:
    ...
    def fillSensor(self):
        ...
        # check for s1
        if self.atWall(locx-GRID,locy-GRID):
            sensors[0] = 1
        else:
            sensors[0] = 0
        # check for s2
        if self.atWall(locx,locy-GRID):
            sensors[1] = 1
        else:
            sensors[1] = 0
        # check for s3
    ...
    def features(self):
        if sensors[1]==1 or sensors[2]==1:
            features[0] = 1
        else:
            features[0] = 0
        if sensors[3]==1 or sensors[4]==1:
            features[1] = 1
        else:
            features[1] = 0
        if sensors[5]==1 or sensors[6]==1:
            features[2] = 1
        else:
            features[2] = 0
        if sensors[7]==1 or sensors[0]==1:
            features[3] = 1
        else:
            features[3] = 0
...
while True:
    # 1 2 3 4 = north south west east
    if features[0]==1 and features[1]==0:
        mybot.go(4)
    elif features[1]==1 and features[2]==0:
        mybot.go(2)
    elif features[2]==1 and features[3]==0:
        mybot.go(3)
    elif features[3]==1 and features[0]==0:
        mybot.go(1)
    else:
