...
void setM( LinearMapR4 &M, float t, float b )
{
  ...
  M.SetColumn4( t*cos(b*t), t*sin(b*t), t, 1 );  		//The curve C(t)
}
...
void display(void)
{
  ...
  const float b = 6;		//constant of Helix curve
  double H = 6.0;
  ...
  for ( float t = 0; t <= 2*3.1415926; t += 0.01 ) {
    get_C ( C, t, b );
     glVertex4fv( C );
  }
  glColor3f ( 0, 0.5, 1 ); 
  ...
  for ( float t = 0.01; t <= 2*3.1415926; t += 0.005 ) {
    setM ( M, t, b );
    for ( int i = 0; i < N; ++i ) {
      p_1 = M * points[i];	//transform the point
